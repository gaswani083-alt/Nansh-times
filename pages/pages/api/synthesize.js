...
// built-in fetch (Node 18+ on Vercel)
const TRUSTED = [
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const items = (search.items || []).filter(i => i.link);
...
${d.text}`).join('\\n\\n')}`;
    ...
    const article = openaiRes.output_text || 'Could not generate article.';
    const bodyHtml = article
      .split('\\n')
      .map(line => line.trim())
      .map(line => line.startsWith('# ') ? `<h2>${escapeHtml(line.slice(2))}</h2>` : (line ? `<p>${escapeHtml(line)}</p>` : ''))
      .join('');
